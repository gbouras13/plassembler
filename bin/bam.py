...
def split_bams(out_dir, threads, logger):
    ...
    try:
        sam_to_bam = sp.Popen(["samtools", "view", "-b", "-h", '-f', '4', "-@", threads, bam], stdout=unmapped_bam, stderr=sp.PIPE) 
        log.write_to_log(sam_to_bam.stderr, logger)
        sam_to_bam_non_chrom = sp.Popen(["samtools", "view", "-b", "-h", "-@", threads, "-L", non_chrom_bed, bam], stdout=non_chrom_bam, stderr=sp.PIPE) 
        log.write_to_log(sam_to_bam_non_chrom.stderr, logger)
        sam_to_bam_chrom = sp.Popen(["samtools", "view", "-b", "-h", "-@", threads, "-L", chrom_bed, bam], stdout=chrom_bam, stderr=sp.PIPE) 
