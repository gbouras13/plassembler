...
def sam_to_bam(out_dir, combo, threads, logger):
    ...
    try:
        ...
        plassemblerModules.write_to_log(sam_to_bam.stderr, logger)
    except:
...
def bam_to_mapped_or_unmapped(out_dir, combo, threads, logger):
    ...
    try:
        ...
        plassemblerModules.write_to_log(bam_to_map.stderr, logger)
    except:
        sys.exit("Error with samtools view. \n")  
...
def extract_long_fastq(out_dir, chrom, logger):
    ...
    try:
        ...
        plassemblerModules.write_to_log(extract_long_fastq.stderr, logger)
    except:
...
def extract_short_fastq(out_dir, chrom, threads, logger):
    ...
    try:
        ...
        plassemblerModules.write_to_log(extract_short_fastq.stdout, logger)
    except:
