...
def get_input():
	...
	parser.add_argument('-t', '--threads', help="Number of threads. Defaults to 1. \n", action="store", default = str(1))
	...
	parser.add_argument('--pacbio_model',  help='Pacbio Flye model. \nMust be one of pacbio-raw, pacbio-corr or pacbio-hifi. \nUse pacbio-raw for PacBio regular CLR reads (<20 percent error), pacbio-corr for PacBio reads that were corrected with other methods (<3 percent error) or pacbio-hifi for PacBio HiFi reads (<1 percent error).', action="store", default='nothing')
	...
	parser.add_argument('--use_raven',  help='Uses Raven instead of Flye for long read assembly. \nMay be useful if you want to reduce runtime.', action="store_true" )
